...
import { Supervivencia, Beneficiario } from '@/lib/types';
import { useRemoverBeneficiarioSupervivencia } from '@/lib/hooks/useSupervivencias';
import { calcularEdad } from '@/lib/utils/formatters';

interface SupervivenciaBeneficiariosProps {
...
export default function SupervivenciaBeneficiarios({
  supervivencia,
  puedeEditar,
  onAgregarClick,
}: SupervivenciaBeneficiariosProps) {
  ...
  const removerMutation = useRemoverBeneficiarioSupervivencia();
  ...
  const handleRemoverConfirm = async () => {
    if (!beneficiarioToDelete || !supervivencia) return;
    try {
  ...
  const beneficiarios = supervivencia.beneficiarios || [];
